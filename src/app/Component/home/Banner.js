...
export default function CarouselComponent() {
  ...
  return (
    <div className="relative overflow-hidden xl:h-[65vh] md:h-[35vh] h-[36vh]">
      {banners.map((banner, index) => (
        <div key={index} className={`absolute top-0 left-0 w-full h-full transition-opacity duration-1000 ease-in-out text-[#1F202F] ${index === activeIndex ? 'opacity-100 z-10' : 'opacity-0 z-0'}`}>
          ...
          <div className="container absolute inset-0 flex flex-col items-center justify-center p-4 mx-auto text-center md:px-10 xl:px-0">
            <h4 className="text-[28px] md:text-[42px] xl:text-[56px] md:leading-[50px] xl:leading-[53px] font-bold">{banner.title}</h4>
            <p className="text-[18px] md:text-[26px] xl:text-[22px] leading-7 mt-1.5 font-light">{banner.subtitle}</p>

            <div className="h-0.5 bg-[#1F202F] w-[15%] my-6"></div>
            <p className="text-[11px] md:text-[16px] max-w-[85ch] font-light">{banner.description}</p>
          </div>
        </div>
      ))}
    </div>
