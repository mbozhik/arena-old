...
export default async function StudentLogin(data, email) {
  try {
    ...
    } else if (response.status === 200 && response.data.login === 1 && response.data.admission === false) {
      const oneDay = 24 * 60 * 60 * 1000
      cookies().set('day_active', response.data.day_active, {path: '/'}, {expires: Date.now() - oneDay})
      cookies().set('email', email, {path: '/'}, {expires: Date.now() - oneDay})
    ...
    } else if (response.status === 200 && response.data.login === 1 && response.data.admission === true) {
      const oneDay = 24 * 60 * 60 * 1000
      cookies().set('uid', response.data.reg_uuid, {path: '/'}, {expires: Date.now() - oneDay})
      cookies().set('email', email, {path: '/'}, {expires: Date.now() - oneDay})
      cookies().set('admission', response.data.admission, {path: '/'}, {expires: Date.now() - oneDay})
