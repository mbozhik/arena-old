...
bKash.init({
    ...
    /* request['amount'] = document.getElementById("tk").value;*/
        ...
        if (request['amount']<500){
            ...
            $.ajax({
                // url: 'http://192.168.0.121:8000/bkash/payment/create/',
    ...
    executeRequestOnAuthorization: function() {
        ...
        $.ajax({
            // url: 'http://192.168.0.121:8000/bkash/payment/execute/?email='+ Email + "&type=" + Type + "&title=" + Course_title,
            ...
            success: function(data) {
                //data = JSON.parse(data);
                if (data && data.paymentID != null) {
                    window.location.href = `/paymentSuccess?trxID=${data.trxID}&amount=${data.amount}`; //MerchantÃ¢â‚¬â„¢s success page
                } else {
